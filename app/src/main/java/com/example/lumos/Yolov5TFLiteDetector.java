...
public class Yolov5TFLiteDetector {
    ...
    private final float IOU_THRESHOLD = 0.9f;
    ...
    public ArrayList<Recognition> detect(Bitmap bitmap) {
        ...
        Log.d("yoohoo", "bitmap height:"+BITMAP_HEIGHT);
        Log.d("yoohoo", "bitmap width:"+BITMAP_WIDTH);

        // yolov5s-tflite的输入是:[1, 320, 320,3], 摄像头每一帧图片需要resize,再归一化
        TensorImage yolov5sTfliteInput;
        ImageProcessor imageProcessor;
        if(IS_INT8){
            Log.d("yoohoo", "in is_int8");
            imageProcessor =
...
//            if(i % 1000 == 0){
//               Log.d("tfliteSupport","x,y,w,h,conf:"+x+","+y+","+w+","+h+","+confidence);
//            }
            int labelId = 0;
            ...
            //Log.d("yoohoo", "Highest confidence score:"+maxLabelScores);
            //Log.d("yoohoo", "Highest confidence Label Id: "+labelId);
...
//                if (allRecognitions.get(j).getLabelId() == i) {
                if (allRecognitions.get(j).getLabelId() == i && allRecognitions.get(j).getConfidence() > DETECT_THRESHOLD) {
                   ...
                   Log.i("yoohoo", "Recognition with confidence>0.25:"+allRecognitions.get(j).toString()+ "i=" +i);
                }
            }
            ...
            while (pq.size() > 0) {
                ...
                for (int k = 1; k < detections.length; k++) {
                    ...
                    if (boxIou(max.getLocation(), detection.getLocation()) < IOU_THRESHOLD) {
                        Log.i("yoohoo", "IOU:"+boxIou(max.getLocation(), detection.getLocation())+"i="+i);
                        pq.add(detection);
                    }
                }
            }
    ...
    protected ArrayList<Recognition> nmsAllClass(ArrayList<Recognition> allRecognitions) {
        ...
        for (int j = 0; j < allRecognitions.size(); ++j) {
            if (allRecognitions.get(j).getConfidence() > DETECT_THRESHOLD) {
                Log.d("yoohoo:","nmsAllClass:"+pq.size());
                Log.d("yoohoo:","nmsAllClass: "+allRecognitions.get(j).getLabelName());
                pq.add(allRecognitions.get(j));
            }
        }
    ...
    protected float boxIou(RectF a, RectF b) {
        ...
        Log.d("yoohoo:","Box intersection:"+intersection);
        float union = boxUnion(a, b);
        Log.d("yoohoo:","Box union:"+union);
        Log.d("yoohoo:","Box intersection/union:"+ intersection/union);
        if (union <= 0) return 1;
